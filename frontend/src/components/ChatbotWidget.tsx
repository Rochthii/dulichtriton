          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50/50">
            {messages.map((msg) => (
              ...
              >
                ...
                <div className={`max-w-[88%] space-y-2 ${msg.sender === 'user' ? 'items-end' : 'items-start'} flex flex-col`}>
                  {/* Text Bubble */}
                  <div
                    className={`rounded-2xl px-4 py-3 text-xs sm:text-sm leading-relaxed shadow-sm ${
                      msg.sender === 'user'
                        ? 'bg-[#1B4D3E] text-white rounded-tr-none font-medium self-end'
                  ...
                  {msg.spots && msg.spots.length > 0 && (
                    <div className="w-full space-y-1.5">
                      {msg.spots.map((spot, i) => (
                        <div
                          key={`${msg.id}-spot-${i}`}
                          className="bg-white rounded-xl border border-slate-200 p-3 shadow-xs hover:border-[#1B4D3E] transition-all flex items-center gap-3"
                        >
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                            src={spot.imageUrl || '/images/tiktok/ho_ta_pa.jpg'}
                            alt={spot.name}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1 text-[10px] font-bold text-[#1B4D3E]">
                              ...
                              <span>{spot.commune}</span>
                            </div>
                            <h4 className="font-bold text-xs text-slate-900 truncate">{spot.name}</h4>
                            <span className="text-[10px] text-slate-500 block truncate">{spot.category}</span>
                          </div>
                          <div className="flex flex-col gap-1 shrink-0">
                            <a
                              ...
                              className="p-1.5 rounded-lg bg-emerald-50 text-[#1B4D3E] hover:bg-[#1B4D3E] hover:text-white transition-colors"
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {msg.suggestions.map((s, i) => (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-full border border-[#1B4D3E]/30 text-[#1B4D3E] text-[10px] font-medium hover:bg-[#1B4D3E] hover:text-white transition-all"
                        >
                          {s}
                        </button>
                      ))}
